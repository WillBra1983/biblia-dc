...
def import_verses():
    ...
    try:
        ...
        with open('NAA.txt', 'r', encoding='utf-8') as file:
            ...
            # Padrão: (número, número, número, número, número, 'texto'),
            pattern = r'\((\d+),(\d+),(\d+),(\d+),(\d+),\'(.*?)\'\),'
            verses = re.findall(pattern, content)
            
            total_verses = 0
            for verse in verses:
                # Extrair os campos (6 campos: id, bible_id, testament_id, book_id, chapter, verse_num, text)
                old_id, bible_id, testament_id, book_id, chapter, verse_num, text = verse
                
                # Inserir o versículo com o novo ID (auto-incrementado)
                cursor.execute(
                    "INSERT INTO verse (book_id, chapter, verse_number, text) VALUES (?, ?, ?, ?)",
                    (int(book_id), int(chapter), int(verse_num), text)
                )
                total_verses += 1
