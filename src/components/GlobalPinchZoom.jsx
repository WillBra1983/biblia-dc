...
function targetInsideNoPinch(target) {
  return (
    target &&
    typeof target.closest === 'function' &&
    target.closest('[data-no-global-pinch]')
  )
}

/**
 * Pinça / Ctrl+scroll no conteúdo principal: até +50% (escala 1.5).
 * Reinicia ao mudar de rota ou quando `zoomResetVersion` incrementa (ex.: Mais de Deus).
 * Conteúdo centrado no eixo horizontal para não cortar à esquerda (planos de leitura).
 */
export default function GlobalPinchZoom({ pathname, zoomResetVersion = 0, disabled, children }) {
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e) => {
      if (targetInsideNoPinch(e.target)) return
  ...
  }, [disabled, pathname, zoomResetVersion])
