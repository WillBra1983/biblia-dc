...
export default function EstudoBiblicoIaPericope() {
  ...
  const linkCompartilharComentario = useMemo(() => {
    ...
    const q = new URLSearchParams({
      ...
      inicio: String(iniQ),
      fim: String(fimQ),
      ...(tituloPericope ? { titulo: tituloPericope } : {}),
    })
    return buildAppShareLink('/estudos-biblicos/ia-pericope', q.toString())
  }, [paramsValidos, livroQ, capQ, iniQ, fimQ, tituloPericope])
  ...
  const votarUtil = useCallback(async () => {
    ...
    try {
      ...
      if (origem === 'novo') {
        const publicacao = await publicarPericopeCandidata({
          livroId: livroQ,
          capitulo: capQ,
          inicio: iniQ,
          fim: fimQ,
      ...
      const voto = await registrarVotoPericope({
      ...
      <Box sx={{ flex: 1, overflow: 'auto', px: { xs: 1, sm: 2 }, py: 2, ...sxLeitura }}>
        ...
        {fase === 'ready' && textoGerado && (
          <>
            ...
            {origem === 'novo' && meuVoto !== 'positivo' && <Paper variant="outlined" sx={{
              mt: 2, p: 2,
