...
export const REGEX_CONF_LINK =
  /\b(CFW|Confissão(?:\s+de\s+Fé)?(?:\s+de\s+Westminster)?|CMW|Catecismo\s+Maior(?:\s+de\s+Westminster)?|CBW|BCW|Breve\s+Catecismo(?:\s+de\s+Westminster)?|Catecismo\s+Breve(?:\s+de\s+Westminster)?|CH|Catecismo\s+de\s+Heidelberg)\*{0,2}\s*(?:[,;:]\s*)?\(?\s*(?:especialmente\s+)?(?:(?:Cap(?:ítulo)?\.?|Perg(?:unta)?s?\.?|Quest(?:ão|ões)?)\s*)?([IVXLC]{1,10}|\d{1,3})(?:\s*(?:(?:[,;:]\s*)?(?:Art(?:igo)?\.?|§)\s*|[.:]\s*)([IVXLC]{1,10}|\d{1,3}))?(?:\s*[-–—]\s*([IVXLC]{1,10}|\d{1,3}))?\)?(?![A-Za-zÀ-ÿ])/gi
